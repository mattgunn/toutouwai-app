...
import { fetchSlackStatus, testSlackConnection, sendSlackNotification } from '../api'
import { Toggle } from './Toggle'
import type { IntegrationConfigProps } from '../types'

const NOTIFICATION_OPTIONS = [
  { key: 'slack_notify_leave', label: 'Leave approvals' },
  { key: 'slack_notify_new_hires', label: 'New hires' },
  { key: 'slack_notify_onboarding', label: 'Onboarding updates' },
  { key: 'slack_notify_anniversaries', label: 'Anniversary reminders' },
] as const

export function SlackConfig({ settings, onSettingsChange }: IntegrationConfigProps) {
  const [status, setStatus] = useState<{ configured: boolean; enabled: boolean; workspace: string } | null>(null)
  ...
  const [webhookUrl, setWebhookUrl] = useState(String(settings.slack_webhook_url || ''))
  const [channel, setChannel] = useState(String(settings.slack_channel || '#hr-notifications'))

  useEffect(() => {
    fetchSlackStatus()
      .then(setStatus)
      .catch(() => setError('Failed to load Slack status'))
      .finally(() => setLoading(false))
  }, [])

  const handleSave = () => {
    onSettingsChange({ slack_webhook_url: webhookUrl, slack_channel: channel })
  ...
  const handleNotify = async () => {
    ...
    try {
      ...
      setNotifyResult(`Test message sent to ${result.channel}`)
    } catch (err: unknown) {
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <Toggle checked={!!settings.slack_enabled} onChange={v => onSettingsChange({ slack_enabled: v })} />
      </div>
      ...
      {!settings.slack_enabled ? (
        ...
        <>
          ...
          <p className="text-xs text-gray-500">
            Send HR notifications to Slack channels — leave approvals, new hires, and more
          </p>
