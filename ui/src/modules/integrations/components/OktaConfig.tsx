...
import { fetchOktaStatus, testOktaConnection, syncOkta, importOkta, pushOkta } from '../api'
import { Toggle } from './Toggle'
import type { IntegrationConfigProps } from '../types'

export function OktaConfig({ settings, onSettingsChange }: IntegrationConfigProps) {
  const [status, setStatus] = useState<{ configured: boolean; enabled: boolean; org_url: string } | null>(null)
  ...
  const [orgUrl, setOrgUrl] = useState(String(settings.okta_org_url || ''))
  const [apiToken, setApiToken] = useState(String(settings.okta_api_token || ''))

  useEffect(() => {
    fetchOktaStatus()
      .then(setStatus)
      .catch(() => setError('Failed to load Okta status'))
      .finally(() => setLoading(false))
  }, [])

  const handleSave = () => {
    onSettingsChange({ okta_org_url: orgUrl, okta_api_token: apiToken })
    setStatus(s => s ? { ...s, configured: !!(apiToken && orgUrl) } : null)
    setTestResult('')
  }
  ...
  const handlePush = async () => {
    ...
    try {
      const result = await pushOkta()
      setSyncResult(`Provisioned: ${result.provisioned}, Deactivated: ${result.deactivated}`)
    } catch (err: unknown) {
    ...
    <div className="bg-gray-900 border border-gray-800 rounded-lg p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h3 className="text-sm font-semibold text-white">Okta</h3>
          {status?.configured ? (
        ...
        <Toggle checked={!!settings.okta_enabled} onChange={v => onSettingsChange({ okta_enabled: v })} />
      </div>

      {!settings.okta_enabled ? (
        ...
        <>
          ...
          <p className="text-xs text-gray-500">
            SCIM user provisioning and SSO via Okta identity platform.
          </p>

          <div className="space-y-2">
            <div>
              <label className="text-xs text-gray-400 block mb-1">Org URL</label>
            ...
            <div>
              ...
              <input type="password" value={apiToken} onChange={e => setApiToken(e.target.value)}
                placeholder="Okta API token from Security → API → Tokens"
                className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-1.5 text-sm text-white placeholder-gray-600" />
            </div>
          </div>
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between py-1">
              <label className="text-xs text-gray-400">SCIM provisioning enabled</label>
              <Toggle
                checked={!!settings.okta_scim_enabled}
                onChange={v => onSettingsChange({ okta_scim_enabled: v })}
              />
            </div>
            <div className="flex items-center justify-between py-1">
              <label className="text-xs text-gray-400">Auto-deactivate on termination</label>
              <Toggle
                checked={!!settings.okta_auto_deactivate}
                onChange={v => onSettingsChange({ okta_auto_deactivate: v })}
              />
            </div>
          </div>

          <div className="flex flex-wrap gap-2 pt-1">
            <button onClick={handleSave} disabled={!orgUrl || !apiToken}
